...
function parseSource(v: unknown, path: string): TextSource {
  ...
  // Mock-output / filled-script / paste-back provenance fields (strings only;
  // never read back to compute anything).
  ...
  const scriptId = asOptStringMax(o.scriptId, `${p}.scriptId`, SOURCE_FIELD_MAX.scriptId);
  if (scriptId !== undefined) src.scriptId = scriptId;
  return src;
}
