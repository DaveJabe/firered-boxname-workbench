...
export function renderReportHtml(project: Project, exportedAtIso: string): string {
  ...
  const blocksHtml = project.importedBlocks
    .map((b) => {
      ...
      const mockLine = s.actionId || s.actionLabel || s.generatedBy
        ? `<p class="muted">Mock action: ${esc(s.actionLabel || s.actionId || '—')}${s.generatedBy ? ' · generated by ' + esc(s.generatedBy) : ''}</p>`
        : '';
      return `<section class="block">
        ...
        ${toolLine}${urlLine}${invLine}${mockLine}
